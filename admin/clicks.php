...
?>
	...
	$sql = "SELECT *, SUM(t1.clicks) as CLICKSUM, SUM(t2.views) as VIEWSUM FROM clicks t1 INNER JOIN views t2 ON t1.banner_id = t2.banner_id WHERE t1.banner_id=" . intval( $BID ) . " AND t1.date >= '{$from}' AND t1.date <= '{$to}' GROUP BY t1.date, t1.clicks, t2.views, t1.block_id, t2.block_id, t2.user_id, t2.date";
